...
def gerer_fin_de_partie(): # Fonction qui détermine le gagnant et affiche le texte de résultat au centre du tapis
    global solde # On déclare solde global pour pouvoir le modifier ici (ajout)
    ...
    if score_j > 21: # Règle 1 : le joueur a dépassé 21, il perd automatiquement
        solde -= mise_actuelle # Le joueur perd sa mise (ajout)
        ...
        solde += mise_actuelle # Le joueur gagne l'équivalent de sa mise (ajout)
        ...
        canvas.create_text(cx, cy, text="DRAW",   font=FONT, fill="gray") # Texte DRAW en gris (mise conservée, ajout)

    # Mise à jour de l'affichage du solde après résultat (ajout)
    canvas.create_text(W - 120, H - 30,
                       text=f"Solde : {solde} €",
                       font=("Helvetica", 20, "bold"), fill="white")
...
def choisir_mise(montant): # Fonction déclenchée quand le joueur clique sur un bouton de mise (ajout)
    global mise_actuelle # On déclare mise_actuelle globale pour pouvoir la modifier ici (ajout)
    if montant <= solde: # On vérifie que le joueur a assez de solde pour cette mise (ajout)
        mise_actuelle = montant # On enregistre la mise choisie (ajout)
        bouton_deal.config(state="normal") # On active le bouton DEAL maintenant qu'une mise est choisie (ajout)
        label_mise.config(text=f"Mise : {mise_actuelle} €") # On met à jour le texte d'info sous les boutons de mise (ajout)
...
def nouvelle_partie(): # Fonction qui réinitialise et lance une nouvelle partie
    ...
    if mise_actuelle == 0: return # Sécurité : on ne lance pas si aucune mise n'a été choisie (ajout)
...
frame_mise = tk.Frame(racine)
frame_mise.grid(row=1, column=0, sticky="w", pady=10, padx=20)

tk.Label(frame_mise, text="Mise :", font=("Helvetica", 14, "bold")).grid(row=0, column=0, padx=5)
for i, montant in enumerate([10, 25, 50, 100]): # On crée un bouton pour chaque montant de mise (ajout)
    tk.Button(frame_mise, text=f"{montant} €", width=6, font=("Helvetica", 13, "bold"),
              command=lambda m=montant: choisir_mise(m)).grid(row=0, column=i+1, padx=5) # Lambda capture le montant à l'itération courante (ajout)
label_mise = tk.Label(frame_mise, text="Mise : 0 €  |  Solde : 1000 €", font=("Helvetica", 13)) # Texte d'info sous les boutons (ajout)
label_mise.grid(row=1, column=0, columnspan=5, pady=3)
...
bouton_deal.config(state="disabled") # Le bouton DEAL est désactivé au démarrage : il faut d'abord choisir une mise (ajout : était normal avant)
