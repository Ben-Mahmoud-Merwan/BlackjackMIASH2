#PARTIE 1 — Merwan Ben mahmoud-Branchut#
# Les fondations du jeu : logique pure du Blackjack #
...
solde = 1000        # Solde de départ du joueur
mise_actuelle = 0   # Mise choisie avant chaque partie (0 = pas encore choisie)
...
#PARTIE 2 — Guillaume Garnier #
#"Le chargement des images et l'affichage sur le tapis"#
...
def actualiser_affichage(fin_partie=False): # Fonction qui efface et redessine tout le tapis (scores + cartes) à chaque action du joueur
    ...
    # Affichage de la mise en bas au centre du tapis en jaune
    canvas.create_text(W // 2, H - 30,
                       text=f"Mise : {mise_actuelle} €",
                       font=("Helvetica", 20, "bold"), fill="yellow")
 
    # Affichage du solde en bas à droite du tapis
    canvas.create_text(W - 120, H - 30,
                       text=f"Solde : {solde} €",
                       font=("Helvetica", 20, "bold"), fill="white")
 ...
 #PARTIE 3 — Younes Mahmoudi #
 #"Les actions du joueur, les règles de fin et la fenêtre tkinter"#

def gerer_fin_de_partie(): # Fonction qui détermine le gagnant et affiche le texte de résultat au centre du tapis
    global solde # On déclare solde global pour pouvoir le modifier ici
    ...
    if score_j > 21: # Règle 1 : le joueur a dépassé 21, il perd automatiquement
        solde -= mise_actuelle # Le joueur perd sa mise
        ...
        solde += mise_actuelle # Le joueur gagne l'équivalent de sa mise
        ...
        canvas.create_text(cx, cy, text="DRAW",   font=FONT, fill="gray") # Texte DRAW en gris (mise conservée)
 
    # Mise à jour de l'affichage du solde après résultat
    canvas.create_text(W - 120, H - 30,
                       text=f"Solde : {solde} €",
                       font=("Helvetica", 20, "bold"), fill="white")
...
def choisir_mise(montant): # Fonction déclenchée quand le joueur clique sur un bouton de mise
    global mise_actuelle # On déclare mise_actuelle globale pour pouvoir la modifier ici
    if montant <= solde: # On vérifie que le joueur a assez de solde pour cette mise
        mise_actuelle = montant # On enregistre la mise choisie
        bouton_deal.config(state="normal") # On active le bouton DEAL maintenant qu'une mise est choisie
        label_mise.config(text=f"Mise : {mise_actuelle} €") # On met à jour le texte d'info sous les boutons de mise
 
 
def nouvelle_partie(): # Fonction qui réinitialise et lance une nouvelle partie
    global paquet, main_joueur, main_croupier, mise_actuelle # On déclare ces variables globales pour pouvoir les modifier ici
    if mise_actuelle == 0: return # Sécurité : on ne lance pas si aucune mise n'a été choisie
...
try:
    racine.state('zoomed') # Ouverture en mode fenêtré maximisé — fonctionne sur Windows et macOS
except Exception:
...
# Conteneur pour les boutons de mise, placé en bas à gauche
frame_mise = tk.Frame(racine) # Création d'un conteneur invisible pour regrouper les boutons de mise
frame_mise.grid(row=1, column=0, sticky="w", pady=10, padx=20) # Placement en bas à gauche de la fenêtre
 
tk.Label(frame_mise, text="Mise :", font=("Helvetica", 14, "bold")).grid(row=0, column=0, padx=5) # Étiquette "Mise :"
for i, montant in enumerate([10, 25, 50, 100]): # On crée un bouton pour chaque montant de mise possible
    tk.Button(frame_mise, text=f"{montant} €", width=6, font=("Helvetica", 13, "bold"),
              command=lambda m=montant: choisir_mise(m)).grid(row=0, column=i+1, padx=5) # Lambda capture le montant à l'itération courante
 
label_mise = tk.Label(frame_mise, text="Mise : 0 €  |  Solde : 1000 €", font=("Helvetica", 13)) # Texte d'info sous les boutons
label_mise.grid(row=1, column=0, columnspan=5, pady=3) # Placement sur la deuxième ligne du conteneur de mise
 
frame_boutons = tk.Frame(racine) # Création d'un conteneur invisible qui regroupe les 3 boutons d'action
...
bouton_deal.config(state="disabled") # Désactivé au démarrage : il faut d'abord choisir une mise
...
bouton_hit.config(state="disabled") # Désactivé au démarrage : il faut d'abord appuyer sur DEAL pour commencer
...
bouton_stand.config(state="disabled") # Désactivé au démarrage : il faut d'abord appuyer sur DEAL pour commencer
